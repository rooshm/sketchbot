...
int main(int argc, char** argv)
{
  ...
  auto visual_tools = std::make_shared<moveit_visual_tools::MoveItVisualTools>(move_group_node, "tool0",
    "sketch_controller", move_group->getRobotModel());
  ...
  auto followpath_cb =
    [&](const std::shared_ptr<sketchbot_interfaces::srv::FollowPath::Request>& request,
      const std::shared_ptr<sketchbot_interfaces::srv::FollowPath::Response>& response) -> bool
    {
      return followpath(request, response, move_group, visual_tools, joint_model_group);
    };

  auto followpath_server =
    move_group_node->create_service<sketchbot_interfaces::srv::FollowPath>("followpath", followpath_cb,
      rmw_qos_profile_services_default, my_callback_group);
